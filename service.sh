...
apply_mode_A(){
  ...
  for cpu in /sys/devices/system/cpu/cpu*/cpufreq; do
    [ -f "$cpu/scaling_max_freq" ] && cat "$cpu/cpuinfo_max_freq" > "$cpu/scaling_max_freq"
    [ -f "$cpu/scaling_min_freq" ] && cat "$cpu/cpuinfo_min_freq" > "$cpu/scaling_min_freq"
  done
  # GPU Max
  ...
  # Thermal relax
  for t in /sys/class/thermal/thermal_zone*/policy; do echo "step_wise" > "$t" 2>/dev/null; done
}
...
apply_mode_D(){
  ...
  SCREEN=$(dumpsys power | grep 'Display Power' | grep -oE 'state=ON|state=OFF')
  case "$SCREEN" in
    *ON*) apply_mode_A;;
    *OFF*) apply_mode_B;;
  esac
...
}(){ log "Applying MODE A – Full BEAST"; }
apply_mode_B(){ log "Applying MODE B – Full Battery Saver"; }
apply_mode_C(){ log "Applying MODE C – Balanced"; }
apply_mode_D(){ log "Applying MODE D – Smart Auto (screen-based)"; }
apply_mode_E(){ log "Applying MODE E – Beast when charging, Saver when unplugged"; }
